...
namespace bluecadet { namespace views {
	...
	typedef std::shared_ptr<class BaseTouchView>		BaseTouchViewRef;
	typedef std::shared_ptr<const class BaseTouchView>	BaseTouchViewConstRef;
	typedef std::weak_ptr<class BaseTouchView>			BaseTouchViewWeakRef;

	//! TouchType options. Touch = fingers on a screen, object = fiducial, mouse = mouse 
	enum TouchType { TOUCH, OBJECT, MOUSE, SIMULATOR };


	class BaseTouchView : public BaseView, public std::enable_shared_from_this<BaseTouchView> {

	public:

		boost::signals2::signal<void(touch::BaseTouchViewRef)>	mDidBeginTouch;		//! Triggered for first touch when touch begins and for subsequent touches if multi-touch is enabled
		boost::signals2::signal<void(touch::BaseTouchViewRef)>	mDidMoveTouch;		//! Triggered for moving touches after touch began
		boost::signals2::signal<void(touch::BaseTouchViewRef)>	mDidEndTouch;		//! Triggered when touch ends and when touch is canceled
		boost::signals2::signal<void(touch::BaseTouchViewRef)>	mDidCancelTouch;	//! Triggered after mDidEndTouch when cancelTouches() is called
		boost::signals2::signal<void(touch::BaseTouchViewRef)>	mDidTap;			//! Triggered after mDidEndTouch if the touch fits the parameters for tapping


		//==================================================
		// Setup/Destruction
		//
		BaseTouchView(bool dragEnabled = false);
		virtual ~BaseTouchView();
		...
		void						setupBaseTouchObj(const cinder::vec2 &size = cinder::vec2(10.0f, 10.0f), bool registerWithTouchManager = true);
		//! Setting up base touch object as a circle
		void						setupBaseTouchObj(float radius = 10.0, bool registerWithTouchManager = true);
