...
void ScreenCamera::setup(ScreenLayoutRef layout) {
	...
	getWindow()->getSignalKeyDown().connect(-1, std::bind(&ScreenCamera::handleKeyDown, this, std::placeholders::_1));
	getWindow()->getSignalResize().connect(-1, std::bind(&ScreenCamera::handleWindowResize, this));
}
